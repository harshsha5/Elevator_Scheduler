...
public:
    ...
    Elevator(std::priority_queue<Passenger, std::vector<Passenger>, Comp> new_pq, int new_floor, DIRN new_direction, int new_assigned_pcount, STATUS new_status,int new_time_since_start, int new_dropoff_time):
    pq(new_pq), floor(new_floor), direction(new_direction), assigned_pcount(new_assigned_pcount), elevator_status(new_status), time_since_start(new_time_since_start), dropoff_time(new_dropoff_time){}

    Elevator(int new_floor): floor(new_floor), direction(IDLE), assigned_pcount(0), elevator_status(NONE),time_since_start(-1),dropoff_time(-1){}
